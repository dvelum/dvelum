<?php return array (
  '_Layout_' => 
  array (
    'id' => '_Layout_',
    'parent' => false,
    'data' => '_Layout_.config.php',
    'order' => 1,
  ),
  '_Component_' => 
  array (
    'id' => '_Component_',
    'parent' => false,
    'data' => '_Component_.config.php',
    'order' => 0,
  ),
  'Groups__docked' => 
  array (
    'id' => 'Groups__docked',
    'parent' => 'Groups',
    'data' => 'Groups__docked.config.php',
    'order' => 0,
  ),
  'Groups_toolbar' => 
  array (
    'id' => 'Groups_toolbar',
    'parent' => 'Groups__docked',
    'data' => 'Groups_toolbar.config.php',
    'order' => 0,
  ),
  'Groups_addGroupBtn__menu' => 
  array (
    'id' => 'Groups_addGroupBtn__menu',
    'parent' => 'Groups_addGroupBtn',
    'data' => 'Groups_addGroupBtn__menu.config.php',
    'order' => 0,
  ),
  'Groups_addGroupBtn' => 
  array (
    'id' => 'Groups_addGroupBtn',
    'parent' => 'Groups_toolbar',
    'data' => 'Groups_addGroupBtn.config.php',
    'order' => 0,
  ),
  'Permissions__docked' => 
  array (
    'id' => 'Permissions__docked',
    'parent' => 'Permissions',
    'data' => 'Permissions__docked.config.php',
    'order' => 0,
  ),
  'Permissions_toolbar' => 
  array (
    'id' => 'Permissions_toolbar',
    'parent' => 'Permissions__docked',
    'data' => 'Permissions_toolbar.config.php',
    'order' => 0,
  ),
  'Permissions_saveBtn__menu' => 
  array (
    'id' => 'Permissions_saveBtn__menu',
    'parent' => 'Permissions_saveBtn',
    'data' => 'Permissions_saveBtn__menu.config.php',
    'order' => 0,
  ),
  'Permissions_saveBtn' => 
  array (
    'id' => 'Permissions_saveBtn',
    'parent' => 'Permissions_toolbar',
    'data' => 'Permissions_saveBtn.config.php',
    'order' => 0,
  ),
  'Users__docked' => 
  array (
    'id' => 'Users__docked',
    'parent' => 'Users',
    'data' => 'Users__docked.config.php',
    'order' => 0,
  ),
  'usersPanel_tools' => 
  array (
    'id' => 'usersPanel_tools',
    'parent' => 'Users__docked',
    'data' => 'usersPanel_tools.config.php',
    'order' => 0,
  ),
  'Permissions' => 
  array (
    'id' => 'Permissions',
    'parent' => '_Component_',
    'data' => 'Permissions.config.php',
    'order' => 2,
  ),
  'permissionsStore' => 
  array (
    'id' => 'permissionsStore',
    'parent' => '_Component_',
    'data' => 'permissionsStore.config.php',
    'order' => 5,
  ),
  'Groups' => 
  array (
    'id' => 'Groups',
    'parent' => '_Component_',
    'data' => 'Groups.config.php',
    'order' => 4,
  ),
  'accountTypesStoreModel' => 
  array (
    'id' => 'accountTypesStoreModel',
    'parent' => '_Component_',
    'data' => 'accountTypesStoreModel.config.php',
    'order' => 11,
  ),
  'statusStoreModel' => 
  array (
    'id' => 'statusStoreModel',
    'parent' => '_Component_',
    'data' => 'statusStoreModel.config.php',
    'order' => 12,
  ),
  'usersStoreModel' => 
  array (
    'id' => 'usersStoreModel',
    'parent' => '_Component_',
    'data' => 'usersStoreModel.config.php',
    'order' => 13,
  ),
  'groupsStore' => 
  array (
    'id' => 'groupsStore',
    'parent' => '_Component_',
    'data' => 'groupsStore.config.php',
    'order' => 10,
  ),
  'mainPanel' => 
  array (
    'id' => 'mainPanel',
    'parent' => '_Component_',
    'data' => 'mainPanel.config.php',
    'order' => 16,
  ),
  'statusStore' => 
  array (
    'id' => 'statusStore',
    'parent' => '_Component_',
    'data' => 'statusStore.config.php',
    'order' => 9,
  ),
  'individualPermissionsStore' => 
  array (
    'id' => 'individualPermissionsStore',
    'parent' => '_Component_',
    'data' => 'individualPermissionsStore.config.php',
    'order' => 6,
  ),
  'accountTypesStore' => 
  array (
    'id' => 'accountTypesStore',
    'parent' => '_Component_',
    'data' => 'accountTypesStore.config.php',
    'order' => 7,
  ),
  'editWindow' => 
  array (
    'id' => 'editWindow',
    'parent' => '_Component_',
    'data' => 'editWindow.config.php',
    'order' => 1,
  ),
  'Users' => 
  array (
    'id' => 'Users',
    'parent' => '_Component_',
    'data' => 'Users.config.php',
    'order' => 0,
  ),
  'groupsStoreModel' => 
  array (
    'id' => 'groupsStoreModel',
    'parent' => '_Component_',
    'data' => 'groupsStoreModel.config.php',
    'order' => 14,
  ),
  'individualPermissions' => 
  array (
    'id' => 'individualPermissions',
    'parent' => '_Component_',
    'data' => 'individualPermissions.config.php',
    'order' => 3,
  ),
  'permissionsStoreModel' => 
  array (
    'id' => 'permissionsStoreModel',
    'parent' => '_Component_',
    'data' => 'permissionsStoreModel.config.php',
    'order' => 15,
  ),
  'usersStore' => 
  array (
    'id' => 'usersStore',
    'parent' => '_Component_',
    'data' => 'usersStore.config.php',
    'order' => 8,
  ),
  'groupsStoreInstance' => 
  array (
    'id' => 'groupsStoreInstance',
    'parent' => '_Layout_',
    'data' => 'groupsStoreInstance.config.php',
    'order' => 1,
  ),
  'statusStoreInstance' => 
  array (
    'id' => 'statusStoreInstance',
    'parent' => '_Layout_',
    'data' => 'statusStoreInstance.config.php',
    'order' => 3,
  ),
  'usersStoreInstance' => 
  array (
    'id' => 'usersStoreInstance',
    'parent' => '_Layout_',
    'data' => 'usersStoreInstance.config.php',
    'order' => 2,
  ),
  'mainPanel_instance' => 
  array (
    'id' => 'mainPanel_instance',
    'parent' => '_Layout_',
    'data' => 'mainPanel_instance.config.php',
    'order' => 0,
  ),
  'accountTypeStoreInstance' => 
  array (
    'id' => 'accountTypeStoreInstance',
    'parent' => '_Layout_',
    'data' => 'accountTypeStoreInstance.config.php',
    'order' => 4,
  ),
  'addUserBtn__menu' => 
  array (
    'id' => 'addUserBtn__menu',
    'parent' => 'addUserBtn',
    'data' => 'addUserBtn__menu.config.php',
    'order' => 0,
  ),
  'editWindow__docked' => 
  array (
    'id' => 'editWindow__docked',
    'parent' => 'editWindow',
    'data' => 'editWindow__docked.config.php',
    'order' => 0,
  ),
  'editWindow_form' => 
  array (
    'id' => 'editWindow_form',
    'parent' => 'editWindow',
    'data' => 'editWindow_form.config.php',
    'order' => 1,
  ),
  'editWindow_bottom_toolbar' => 
  array (
    'id' => 'editWindow_bottom_toolbar',
    'parent' => 'editWindow__docked',
    'data' => 'editWindow_bottom_toolbar.config.php',
    'order' => 0,
  ),
  'editWindow_footer_saveBtn' => 
  array (
    'id' => 'editWindow_footer_saveBtn',
    'parent' => 'editWindow_bottom_toolbar',
    'data' => 'editWindow_footer_saveBtn.config.php',
    'order' => 1,
  ),
  'editWindow_footer_cancelBtn' => 
  array (
    'id' => 'editWindow_footer_cancelBtn',
    'parent' => 'editWindow_bottom_toolbar',
    'data' => 'editWindow_footer_cancelBtn.config.php',
    'order' => 2,
  ),
  'editWindow_footer_fill' => 
  array (
    'id' => 'editWindow_footer_fill',
    'parent' => 'editWindow_bottom_toolbar',
    'data' => 'editWindow_footer_fill.config.php',
    'order' => 0,
  ),
  'editWindow_password_confirm' => 
  array (
    'id' => 'editWindow_password_confirm',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_password_confirm.config.php',
    'order' => 9,
  ),
  'editWindow_group_id' => 
  array (
    'id' => 'editWindow_group_id',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_group_id.config.php',
    'order' => 2,
  ),
  'editWindow_admin' => 
  array (
    'id' => 'editWindow_admin',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_admin.config.php',
    'order' => 1,
  ),
  'editWindow_password' => 
  array (
    'id' => 'editWindow_password',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_password.config.php',
    'order' => 8,
  ),
  'editWindow_setpass' => 
  array (
    'id' => 'editWindow_setpass',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_setpass.config.php',
    'order' => 7,
  ),
  'editWindow_enabled' => 
  array (
    'id' => 'editWindow_enabled',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_enabled.config.php',
    'order' => 6,
  ),
  'editWindow_email' => 
  array (
    'id' => 'editWindow_email',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_email.config.php',
    'order' => 5,
  ),
  'editWindow_login' => 
  array (
    'id' => 'editWindow_login',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_login.config.php',
    'order' => 4,
  ),
  'editWindow_name' => 
  array (
    'id' => 'editWindow_name',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_name.config.php',
    'order' => 3,
  ),
  'editWindow_id' => 
  array (
    'id' => 'editWindow_id',
    'parent' => 'editWindow_form',
    'data' => 'editWindow_id.config.php',
    'order' => 0,
  ),
  'individualPermissions__docked' => 
  array (
    'id' => 'individualPermissions__docked',
    'parent' => 'individualPermissions',
    'data' => 'individualPermissions__docked.config.php',
    'order' => 0,
  ),
  'ip_userFilterLabel' => 
  array (
    'id' => 'ip_userFilterLabel',
    'parent' => 'individualPermissionsTools',
    'data' => 'ip_userFilterLabel.config.php',
    'order' => 0,
  ),
  'ip_userFilter' => 
  array (
    'id' => 'ip_userFilter',
    'parent' => 'individualPermissionsTools',
    'data' => 'ip_userFilter.config.php',
    'order' => 1,
  ),
  'saveIndividualPermissionsBtn' => 
  array (
    'id' => 'saveIndividualPermissionsBtn',
    'parent' => 'individualPermissionsTools',
    'data' => 'saveIndividualPermissionsBtn.config.php',
    'order' => 2,
  ),
  'individualPermissionsTools' => 
  array (
    'id' => 'individualPermissionsTools',
    'parent' => 'individualPermissions__docked',
    'data' => 'individualPermissionsTools.config.php',
    'order' => 0,
  ),
  'mainPanel__docked' => 
  array (
    'id' => 'mainPanel__docked',
    'parent' => 'mainPanel',
    'data' => 'mainPanel__docked.config.php',
    'order' => 0,
  ),
  'usersPanel_sep1' => 
  array (
    'id' => 'usersPanel_sep1',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_sep1.config.php',
    'order' => 1,
  ),
  'addUserBtn' => 
  array (
    'id' => 'addUserBtn',
    'parent' => 'usersPanel_tools',
    'data' => 'addUserBtn.config.php',
    'order' => 0,
  ),
  'usersPanel_accountFilter' => 
  array (
    'id' => 'usersPanel_accountFilter',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_accountFilter.config.php',
    'order' => 3,
  ),
  'usersPanel_groupFilter' => 
  array (
    'id' => 'usersPanel_groupFilter',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_groupFilter.config.php',
    'order' => 8,
  ),
  'usersPanel_groupLabel' => 
  array (
    'id' => 'usersPanel_groupLabel',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_groupLabel.config.php',
    'order' => 7,
  ),
  'usersPanel_accountFilterLabel' => 
  array (
    'id' => 'usersPanel_accountFilterLabel',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_accountFilterLabel.config.php',
    'order' => 2,
  ),
  'usersPanel_sep2' => 
  array (
    'id' => 'usersPanel_sep2',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_sep2.config.php',
    'order' => 4,
  ),
  'usersPanel_statusFilterLabel' => 
  array (
    'id' => 'usersPanel_statusFilterLabel',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_statusFilterLabel.config.php',
    'order' => 5,
  ),
  'usersPanel_fill' => 
  array (
    'id' => 'usersPanel_fill',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_fill.config.php',
    'order' => 9,
  ),
  'usersPanel_statusFilter' => 
  array (
    'id' => 'usersPanel_statusFilter',
    'parent' => 'usersPanel_tools',
    'data' => 'usersPanel_statusFilter.config.php',
    'order' => 6,
  ),
  'userSearch' => 
  array (
    'id' => 'userSearch',
    'parent' => 'usersPanel_tools',
    'data' => 'userSearch.config.php',
    'order' => 10,
  ),
); 