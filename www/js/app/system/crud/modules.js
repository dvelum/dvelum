...
Ext.define('app.crud.modules.Main',{
	...
	initComponent: function(){
		...
		var columns = [
			{
				...
				flex:1,
				widget: {
					defaultCellUI:"default",
					defaultBindProperty:'value',
					xtype:'interfaceField',
					controllerUrl:me.controllerUrl,
					listeners:{
						completeEdit:{
							fn:function(value , cmp){
								cmp.getWidgetRecord().set('designer' , value);
							},
							scope:me
						}
					}
			...
			//,
				...
				//	me.dataGrid.getView().refreshNode(record);
			//	}
			},{
				text:appLang.DEV,
				dataIndex: 'dev',
		...
		//
		// @todo Fix layout issue without defered refresh
		//
		this.dataStore.on('load',function(){
			var me = this;
			setTimeout(function(){
				me.dataGrid.getView().refresh();
			}, 100);
		},this);
